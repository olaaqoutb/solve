    ...
    Object.keys(groupedByMonth).sort((a, b) => {
      ...
      Object.keys(groupedByDay).sort((a, b) => {
       ...
       const dayNode: any = {
        name: dayKey,
        dayName: dayKey,
        gestempelt: dayTotal, // This is the value needed for Level 1
        gebucht: dayTotal,
        hasNotification: false,
        ...
        // --- CHANGE HERE: Swapped order (Time Range first, then Text) ---
...
onNodeDoubleClick(node: FlatNode, event?: MouseEvent) {
  // Second click detected - clear timeout and handle double click
...
private handleSingleClick(node: FlatNode) {
  ...
  // Single click: Show form in right panel for current level
...
private handleDoubleClick(node: FlatNode) {
  ...
  // Double click: Expand/collapse the node if it's expandable
  if (node.expandable) {
  ...
  // Also select and show the node (same as single click)
  this.handleSingleClick(node);
}
...
// FIXED: adjustTime to check editing state
adjustTime(type: 'start' | 'end', unit: 'hour' | 'minute', amount: number) {
  ...
  if (control) {
    ...
    if (unit === 'hour') {
      ...
      // If hour becomes 24, force minutes to 0
      if (newVal === 24) {
...
// Add this method to properly handle post-creation state
