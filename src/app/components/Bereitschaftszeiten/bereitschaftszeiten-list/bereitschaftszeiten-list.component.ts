...
@Component({
  selector: 'app-bereitschaftszeiten-list',
  imports: [ FormsModule,
    MatTableModule,
    MatSortModule,
    MatIconModule,
    ...
    MatCheckboxModule,
    HttpClientModule],
      templateUrl: './bereitschaftszeiten-list.component.html',
  styleUrl: './bereitschaftszeiten-list.component.scss'
})
export class BereitschaftszeitenListComponent {
...
private applySorting(data: Person[]): Person[] {
 ...
 goToDetails(row: Person): void {
  ...
  if (row.id) {
    this.router.navigate(['/standby-two', row.id]);
  } else {
